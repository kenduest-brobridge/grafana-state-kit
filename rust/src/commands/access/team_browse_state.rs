...
#[cfg(test)]
mod tests {
    use super::{row_kind, BrowserState, SearchDirection, SearchState};
    use serde_json::{Map, Value};

    fn team_row(id: &str, name: &str, email: &str, members: &str) -> Map<String, Value> {
        Map::from_iter(vec![
            ("id".to_string(), Value::String(id.to_string())),
            ("name".to_string(), Value::String(name.to_string())),
            ("email".to_string(), Value::String(email.to_string())),
            ("members".to_string(), Value::String(members.to_string())),
        ])
    }
    ...
    fn repeat_search_state() -> BrowserState {
        BrowserState::new(vec![
            team_row("1", "platform-ops", "ops@example.test", "alice"),
            team_row("2", "billing", "billing@example.test", "bob"),
            team_row("3", "incident-ops", "incident@example.test", "carol"),
        ])
    }
    ...
    #[test]
    fn team_browse_repeat_last_search_wraps_forward_to_first_match() {
    ...
    #[test]
    fn team_browse_repeat_last_search_wraps_backward_to_last_match() {
        let mut state = repeat_search_state();
    ...
    #[test]
    fn team_browse_repeat_last_search_without_previous_search_does_not_select() {
        let mut state = repeat_search_state();
        state.select_index(1);
    ...
    #[test]
    fn team_browse_repeat_last_search_empty_query_or_no_match_does_not_select() {
        let mut state = repeat_search_state();
        state.select_index(1);
