...
- Current Update: Aligned access user/team, datasource browse, and shared read-only browser `n` repeat-search with dashboard and status overview behavior by skipping the current row first, then wrapping forward to the first match or backward to the last match; added a datasource Review pane for secret placeholder, provider, read-only blocker, review-required evidence, and compatible local plan/diff/import evidence fields already present in datasource details; reused those safe Review lines in local `datasource list --interactive` item details; aligned dashboard inspect workbench footer exit copy with the compact `Esc/q` convention.
- Result: Browse search now moves consistently through result sets and active filters, and datasource browse/list interactive views keep general metadata in Facts/details while surfacing safe operational review evidence without resolved credential values, raw provider tokens, or secret-like changed-field paths.
